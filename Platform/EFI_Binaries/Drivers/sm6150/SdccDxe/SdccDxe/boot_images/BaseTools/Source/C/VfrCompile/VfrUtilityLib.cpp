...
VOID CVfrBinaryOutput::WriteLine(IN FILE *pFile, IN UINT32 LineBytes,
                                 IN CONST CHAR8 *LineHeader, IN CHAR8 *BlkBuf,
                                 IN UINT32 BlkSize) {
  UINT32 Index;
...
VOID CVfrBinaryOutput::WriteEnd(IN FILE *pFile, IN UINT32 LineBytes,
                                IN CONST CHAR8 *LineHeader, IN CHAR8 *BlkBuf,
                                IN UINT32 BlkSize) {
  UINT32 Index;

  if ((BlkSize == 0) || (pFile == NULL) || (LineHeader == NULL) ||
      (BlkBuf == NULL)) {
    return;
  }
...
SConfigInfo::SConfigInfo(IN UINT8 Type, IN UINT16 Offset, IN UINT32 Width,
...
SConfigInfo::~SConfigInfo(VOID) { BUFFER_SAFE_FREE(mValue); }

SConfigItem::SConfigItem(IN CHAR8 *Name, IN EFI_GUID *Guid, IN CHAR8 *Id) {
  mName = NULL;
  mGuid = NULL;
  mId = NULL;
  mInfoStrList = NULL;
...
SConfigItem::SConfigItem(IN CHAR8 *Name, IN EFI_GUID *Guid, IN CHAR8 *Id,
                         IN UINT8 Type, IN UINT16 Offset, IN UINT16 Width,
                         IN EFI_IFR_TYPE_VALUE Value) {
  mName = NULL;
  mGuid = NULL;
  mId = NULL;
...
UINT8
CVfrBufferConfig::Register(IN CHAR8 *Name, IN EFI_GUID *Guid, IN CHAR8 *Id) {
  SConfigItem *pNew;
...
VOID CVfrBufferConfig::Open(VOID) { mItemListPos = mItemListHead; }

BOOLEAN
CVfrBufferConfig::Eof(VOID) { return (mItemListPos == NULL) ? TRUE : FALSE; }

UINT8
CVfrBufferConfig::Select(IN CHAR8 *Name, IN EFI_GUID *Guid, IN CHAR8 *Id) {
  SConfigItem *p;
  ...
  if (Name == NULL || Guid == NULL) {
    ...
    for (p = mItemListHead; p != NULL; p = p->mNext) {
      if ((strcmp(p->mName, Name) != 0) ||
          (memcmp(p->mGuid, Guid, sizeof(EFI_GUID)) != 0)) {
        continue;
      }
...
UINT8
CVfrBufferConfig::Write(IN CONST CHAR8 Mode, IN CHAR8 *Name, IN EFI_GUID *Guid,
                        IN CHAR8 *Id, IN UINT8 Type, IN UINT16 Offset,
                        IN UINT32 Width, IN EFI_IFR_TYPE_VALUE Value) {
  UINT8 Ret;
  SConfigItem *pItem;
  SConfigInfo *pInfo;
  ...
  case 'a': // add
    if (Select(Name, Guid, Id) != 0) {
      if ((pItem = new SConfigItem(Name, Guid, Id, Type, Offset, (UINT16)Width,
                                   Value)) == NULL) {
        return 2;
      }
      if (mItemListHead == NULL) {
      ...
      // same offset
      for (pInfo = mItemListPos->mInfoStrList; pInfo != NULL;
           pInfo = pInfo->mNext) {
  ...
  case 'd': // delete
    ...
    for (pItem = mItemListHead; pItem->mNext != mItemListPos;
         pItem = pItem->mNext)
      ;
...
VOID CVfrBufferConfig::Close(VOID) { mItemListPos = NULL; }

#define BYTES_PRE_LINE 0x10

VOID CVfrBufferConfig::OutputCFile(IN FILE *pFile, IN CHAR8 *BaseName) {
  ...
  for (Item = mItemListHead; Item != NULL; Item = Item->mNext) {
    ...
    Output.WriteLine(pFile, BYTES_PRE_LINE, "  ", (CHAR8 *)&TotalLen,
                     sizeof(UINT32));

    for (Info = Item->mInfoStrList; Info != NULL; Info = Info->mNext) {
      fprintf(pFile, "\n");
      Output.WriteLine(pFile, BYTES_PRE_LINE, "  ", (CHAR8 *)&Info->mOffset,
                       sizeof(UINT16));
      Output.WriteLine(pFile, BYTES_PRE_LINE, "  ", (CHAR8 *)&Info->mWidth,
                       sizeof(UINT16));
    }
    fprintf(pFile, "\n};\n");
  }
    ...
    if (Item->mId != NULL && Item->mInfoStrList != NULL) {
      ...
      for (Info = Item->mInfoStrList; Info != NULL; Info = Info->mNext) {
        ...
        Output.WriteLine(pFile, BYTES_PRE_LINE, "  ", (CHAR8 *)&Info->mWidth,
                         sizeof(UINT16));
        if (Info->mNext == NULL) {
          Output.WriteEnd(pFile, BYTES_PRE_LINE, "  ", (CHAR8 *)Info->mValue,
                          Info->mWidth);
        } else {
          Output.WriteLine(pFile, BYTES_PRE_LINE, "  ", (CHAR8 *)Info->mValue,
                           Info->mWidth);
        }
      }
      fprintf(pFile, "\n};\n");
...
CVfrBufferConfig::CVfrBufferConfig(VOID) {
  mItemListHead = NULL;
  mItemListTail = NULL;
  mItemListPos = NULL;
}
...
static struct {
  ...
  UINT32 mAlign;
    ...
    {"EFI_STRING_ID", EFI_IFR_TYPE_STRING, sizeof(EFI_STRING_ID),
...
STATIC
BOOLEAN
_IS_INTERNAL_TYPE(IN CHAR8 *TypeName) {
  UINT32 Index;
...
STATIC
CHAR8 *TrimHex(IN CHAR8 *Str, OUT bool *IsHex) {
  *IsHex = FALSE;
...
VOID CVfrVarDataTypeDB::RegisterNewType(IN SVfrDataType *New) {
  New->mNext = mDataTypeList;
  mDataTypeList = New;
}

EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::ExtractStructTypeName(IN CHAR8 *&VarStr, OUT CHAR8 *TName) {
  if (TName == NULL) {
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::ExtractFieldNameAndArrary(IN CHAR8 *&VarStr, IN CHAR8 *FName,
                                             OUT UINT32 &ArrayIdx) {
  UINT32 Idx;
  ...
  *FName = '\0';
  ...
  case '[':
    VarStr++;
    for (Idx = 0; (Idx < MAX_NAME_LEN) && (*VarStr != '\0') && (*VarStr != ']');
         VarStr++, Idx++) {
      ArrayStr[Idx] = *VarStr;
    }
    ArrayStr[Idx] = '\0';
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetTypeField(IN CONST CHAR8 *FName, IN SVfrDataType *Type,
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetFieldOffset(IN SVfrDataField *Field, IN UINT32 ArrayIdx,
                                  OUT UINT32 &Offset) {
  if (Field == NULL) {
    return VFR_RETURN_FATAL_ERROR;
  }
  ...
  if ((ArrayIdx != INVALID_ARRAY_INDEX) &&
      ((Field->mArrayNum == 0) || (Field->mArrayNum <= ArrayIdx))) {
    return VFR_RETURN_ERROR_ARRARY_NUM;
  }
  ...
  Offset =
      Field->mOffset + Field->mFieldType->mTotalSize *
                           ((ArrayIdx == INVALID_ARRAY_INDEX) ? 0 : ArrayIdx);
  return VFR_RETURN_SUCCESS;
}

UINT8
CVfrVarDataTypeDB::GetFieldWidth(IN SVfrDataField *Field) {
  if (Field == NULL) {
    return 0;
  }
...
UINT32
CVfrVarDataTypeDB::GetFieldSize(IN SVfrDataField *Field, IN UINT32 ArrayIdx) {
  if (Field == NULL) {
    return VFR_RETURN_FATAL_ERROR;
  }
...
VOID CVfrVarDataTypeDB::InternalTypesListInit(VOID) {
...
CVfrVarDataTypeDB::CVfrVarDataTypeDB(VOID) {
  mDataTypeList = NULL;
  mNewDataType = NULL;
  mCurrDataField = NULL;
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::Pack(IN UINT32 LineNum, IN UINT8 Action,
                        IN CHAR8 *Identifier, IN UINT32 Number) {
  UINT32 PackAlign;
  ...
  if (Action & VFR_PACK_POP) {
    ...
    if (mPackStack == NULL) {
      gCVfrErrorHandle.PrintMsg(LineNum, NULL, "Error",
...
VOID CVfrVarDataTypeDB::DeclareDataTypeBegin(VOID) {
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::SetNewTypeName(IN CHAR8 *TypeName) {
  SVfrDataType *pType;
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::DataTypeAddField(IN CHAR8 *FieldName, IN CHAR8 *TypeName,
                                    IN UINT32 ArrayNum) {
  ...
  if ((mNewDataType->mTotalSize % Align) == 0) {
    ...
    pNewField->mOffset =
        mNewDataType->mTotalSize + ALIGN_STUFF(mNewDataType->mTotalSize, Align);
  }
  ...
  mNewDataType->mAlign =
      MIN(mPackAlign, MAX(pFieldType->mAlign, mNewDataType->mAlign));
...
VOID CVfrVarDataTypeDB::DeclareDataTypeEnd(VOID) {
  ...
  if ((mNewDataType->mTotalSize % mNewDataType->mAlign) != 0) {
    mNewDataType->mTotalSize +=
        ALIGN_STUFF(mNewDataType->mTotalSize, mNewDataType->mAlign);
  }
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetDataType(IN CHAR8 *TypeName,
  ...
  *DataType = NULL;
  ...
  for (pDataType = mDataTypeList; pDataType != NULL;
       pDataType = pDataType->mNext) {
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetDataTypeSize(IN UINT8 DataType, OUT UINT32 *Size) {
  SVfrDataType *pDataType = NULL;
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetDataTypeSize(IN CHAR8 *TypeName, OUT UINT32 *Size) {
  SVfrDataType *pDataType = NULL;
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetDataFieldInfo(IN CHAR8 *VarStr, OUT UINT16 &Offset,
                                    OUT UINT8 &Type, OUT UINT32 &Size) {
...
EFI_VFR_RETURN_CODE
CVfrVarDataTypeDB::GetUserDefinedTypeNameList(OUT CHAR8 ***NameList,
                                              OUT UINT32 *ListSize) {
  UINT32 Index;
  SVfrDataType *pType;
  ...
  *ListSize = 0;
  ...
  for (Index = 0, pType = mDataTypeList; pType != NULL;
       pType = pType->mNext, Index++) {
    if (_IS_INTERNAL_TYPE(pType->mTypeName) == FALSE) {
...
BOOLEAN
CVfrVarDataTypeDB::IsTypeNameDefined(IN CHAR8 *TypeName) {
  SVfrDataType *pType;
...
VOID CVfrVarDataTypeDB::Dump(IN FILE *File) {
  SVfrDataType *pTNode;
    ...
    for (pFNode = pTNode->mMembers; pFNode != NULL; pFNode = pFNode->mNext) {
      if (pFNode->mArrayNum > 0) {
        fprintf(File, "\t\t\t+%08d[%08x] %s[%d] <%s>\n", pFNode->mOffset,
                pFNode->mOffset, pFNode->mFieldName, pFNode->mArrayNum,
                pFNode->mFieldType->mTypeName);
      } else {
        fprintf(File, "\t\t\t+%08d[%08x] %s <%s>\n", pFNode->mOffset,
                pFNode->mOffset, pFNode->mFieldName,
                pFNode->mFieldType->mTypeName);
      }
    }
    fprintf(File, "\t\t};\n");
...
VOID CVfrVarDataTypeDB::ParserDB(VOID) {
  SVfrDataType *pTNode;
  SVfrDataField *pFNode;
  ...
  for (pTNode = mDataTypeList; pTNode != NULL; pTNode = pTNode->mNext) {
    printf("\t\tstruct %s : mAlign [%x] mTotalSize [%x]\n\n", pTNode->mTypeName,
           pTNode->mAlign, pTNode->mTotalSize);
    printf("\t\tstruct %s {\n", pTNode->mTypeName);
    for (pFNode = pTNode->mMembers; pFNode != NULL; pFNode = pFNode->mNext) {
      printf("\t\t\t%s\t%s\n", pFNode->mFieldType->mTypeName,
             pFNode->mFieldName);
...
SVfrVarStorageNode::SVfrVarStorageNode(IN EFI_GUID *Guid, IN CHAR8 *StoreName,
                                       IN EFI_VARSTORE_ID VarStoreId,
                                       IN EFI_STRING_ID VarName,
                                       IN UINT32 VarSize, IN BOOLEAN Flag) {
  if (Guid != NULL) {
    mGuid = *Guid;
  } else {
...
SVfrVarStorageNode::SVfrVarStorageNode(IN CHAR8 *StoreName,
                                       IN EFI_VARSTORE_ID VarStoreId) {
  if (StoreName != NULL) {
    mVarStoreName = new CHAR8[strlen(StoreName) + 1];
    strcpy(mVarStoreName, StoreName);
  ...
  mStorageInfo.mNameSpace.mNameTable =
      new EFI_VARSTORE_ID[DEFAULT_NAME_TABLE_ITEMS];
  mStorageInfo.mNameSpace.mTableSize = 0;
}

SVfrVarStorageNode::~SVfrVarStorageNode(VOID) {
...
CVfrDataStorage::CVfrDataStorage(VOID) {
  UINT32 Index;
...
EFI_VARSTORE_ID
CVfrDataStorage::GetFreeVarStoreId(EFI_VFR_VARSTORE_TYPE VarType) {
...
BOOLEAN
CVfrDataStorage::ChekVarStoreIdFree(IN EFI_VARSTORE_ID VarStoreId) {
...
VOID CVfrDataStorage::MarkVarStoreIdUsed(IN EFI_VARSTORE_ID VarStoreId) {
  UINT32 Index = (VarStoreId / EFI_BITS_PER_UINT32);
  UINT32 Offset = (VarStoreId % EFI_BITS_PER_UINT32);
...
VOID CVfrDataStorage::MarkVarStoreIdUnused(IN EFI_VARSTORE_ID VarStoreId) {
  UINT32 Index = (VarStoreId / EFI_BITS_PER_UINT32);
  UINT32 Offset = (VarStoreId % EFI_BITS_PER_UINT32);
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::DeclareNameVarStoreBegin(IN CHAR8 *StoreName,
                                          IN EFI_VARSTORE_ID VarStoreId) {
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::NameTableAddItem(IN EFI_STRING_ID Item) {
  ...
  if ((TableSize != 0) && ((TableSize % DEFAULT_NAME_TABLE_ITEMS) == 0)) {
    if ((NewTable =
             new EFI_VARSTORE_ID[TableSize + DEFAULT_NAME_TABLE_ITEMS]) ==
        NULL) {
      return VFR_RETURN_OUT_FOR_RESOURCES;
    }
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::DeclareNameVarStoreEnd(IN EFI_GUID *Guid) {
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::DeclareEfiVarStore(IN CHAR8 *StoreName, IN EFI_GUID *Guid,
  ...
  if ((pNode = new SVfrVarStorageNode(Guid, StoreName, VarStoreId, NameStrId,
                                      VarSize, Flag)) == NULL) {
    return VFR_RETURN_OUT_FOR_RESOURCES;
  }
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::DeclareBufferVarStore(IN CHAR8 *StoreName, IN EFI_GUID *Guid,
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::GetVarStoreByDataType(IN CHAR8 *DataTypeName,
  ...
  //
  // Framework VFR uses Data type name as varstore name, so don't need check
  // again.
  //
...
EFI_VARSTORE_ID
CVfrDataStorage::CheckGuidField(IN SVfrVarStorageNode *pNode,
                                IN EFI_GUID *StoreGuid, IN BOOLEAN *HasFoundOne,
...
**/
EFI_VFR_RETURN_CODE
CVfrDataStorage::GetVarStoreId(IN CHAR8 *StoreName,
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::GetBufferVarStoreDataTypeName(IN EFI_VARSTORE_ID VarStoreId,
...
CVfrDataStorage::GetVarStoreType(IN EFI_VARSTORE_ID VarStoreId) {
  SVfrVarStorageNode *pNode;
...
EFI_GUID *CVfrDataStorage::GetVarStoreGuid(IN EFI_VARSTORE_ID VarStoreId) {
  SVfrVarStorageNode *pNode;
  EFI_GUID *VarGuid;
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::GetVarStoreName(IN EFI_VARSTORE_ID VarStoreId,
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::GetEfiVarStoreInfo(IN OUT EFI_VARSTORE_INFO *Info) {
  if (Info == NULL) {
    return VFR_RETURN_FATAL_ERROR;
  }
...
EFI_VFR_RETURN_CODE
CVfrDataStorage::GetNameVarStoreInfo(OUT EFI_VARSTORE_INFO *Info,
                                     IN UINT32 Index) {
  if (Info == NULL) {
    return VFR_RETURN_FATAL_ERROR;
  }
  ...
  Info->mInfo.mVarName =
      mCurrVarStorageNode->mStorageInfo.mNameSpace.mNameTable[Index];

  return VFR_RETURN_SUCCESS;
}

SVfrDefaultStoreNode::SVfrDefaultStoreNode(IN EFI_IFR_DEFAULTSTORE *ObjBinAddr,
                                           IN CHAR8 *RefName,
...
SVfrDefaultStoreNode::~SVfrDefaultStoreNode(VOID) {
  if (mRefName != NULL) {
    delete mRefName;
  }
}

CVfrDefaultStore::CVfrDefaultStore(VOID) { mDefaultStoreList = NULL; }

CVfrDefaultStore::~CVfrDefaultStore(VOID) {
...
EFI_VFR_RETURN_CODE
CVfrDefaultStore::RegisterDefaultStore(IN CHAR8 *ObjBinAddr, IN CHAR8 *RefName,
                                       IN EFI_STRING_ID DefaultStoreNameId,
                                       IN UINT16 DefaultId) {
  ...
  if ((pNode =
           new SVfrDefaultStoreNode((EFI_IFR_DEFAULTSTORE *)ObjBinAddr, RefName,
                                    DefaultStoreNameId, DefaultId)) == NULL) {
    return VFR_RETURN_OUT_FOR_RESOURCES;
  }
...
CVfrDefaultStore::ReRegisterDefaultStoreById(
    IN UINT16 DefaultId, IN CHAR8 *RefName,
    IN EFI_STRING_ID DefaultStoreNameId) {
  SVfrDefaultStoreNode *pNode = NULL;
...
BOOLEAN
CVfrDefaultStore::DefaultIdRegistered(IN UINT16 DefaultId) {
  SVfrDefaultStoreNode *pNode = NULL;
...
EFI_VFR_RETURN_CODE
CVfrDefaultStore::GetDefaultId(IN CHAR8 *RefName, OUT UINT16 *DefaultId) {
  SVfrDefaultStoreNode *pTmp = NULL;
...
EFI_VFR_RETURN_CODE
CVfrDefaultStore::BufferVarStoreAltConfigAdd(IN EFI_VARSTORE_ID DefaultId,
                                             ...
                                             IN EFI_IFR_TYPE_VALUE Value) {
  SVfrDefaultStoreNode *pNode = NULL;
  CHAR8 NewAltCfg[2 * 2 * sizeof(UINT16) + 1] = {
      0,
  };
  INTN Returnvalue = 0;
  ...
  if ((Returnvalue = gCVfrBufferConfig.Select(VarStoreName, VarStoreGuid)) ==
      0) {
    if ((Returnvalue = gCVfrBufferConfig.Write(
             'a', VarStoreName, VarStoreGuid, NewAltCfg, Type,
             Info.mInfo.mVarOffset, Info.mVarTotalSize, Value)) != 0) {
      goto WriteError;
    }
  }
...
SVfrRuleNode::SVfrRuleNode(IN CHAR8 *RuleName, IN UINT8 RuleId) {
  if (RuleName != NULL) {
...
SVfrRuleNode::~SVfrRuleNode(VOID) {
  if (mRuleName != NULL) {
    delete mRuleName;
  }
}
...
VOID CVfrRulesDB::RegisterRule(IN CHAR8 *RuleName) {
  SVfrRuleNode *pNew;
...
UINT8
CVfrRulesDB::GetRuleId(IN CHAR8 *RuleName) {
  SVfrRuleNode *pNode;
...
EFI_VARSTORE_INFO::EFI_VARSTORE_INFO(VOID) {
...
EFI_VARSTORE_INFO::EFI_VARSTORE_INFO(IN EFI_VARSTORE_INFO &Info) {
...
BOOLEAN
EFI_VARSTORE_INFO::operator==(IN EFI_VARSTORE_INFO *Info) {
  if ((mVarStoreId == Info->mVarStoreId) &&
      (mInfo.mVarName == Info->mInfo.mVarName) &&
      (mInfo.mVarOffset == Info->mInfo.mVarOffset) &&
      (mVarType == Info->mVarType) && (mVarTotalSize == Info->mVarTotalSize)) {
    return TRUE;
  }
...
EFI_QUESTION_ID
CVfrQuestionDB::GetFreeQuestionId(VOID) {
  UINT32 Index, Mask, Offset;
...
BOOLEAN
CVfrQuestionDB::ChekQuestionIdFree(IN EFI_QUESTION_ID QId) {
...
VOID CVfrQuestionDB::MarkQuestionIdUsed(IN EFI_QUESTION_ID QId) {
  UINT32 Index = (QId / EFI_BITS_PER_UINT32);
  UINT32 Offset = (QId % EFI_BITS_PER_UINT32);
...
VOID CVfrQuestionDB::MarkQuestionIdUnused(IN EFI_QUESTION_ID QId) {
  UINT32 Index = (QId / EFI_BITS_PER_UINT32);
  UINT32 Offset = (QId % EFI_BITS_PER_UINT32);
...
SVfrQuestionNode::SVfrQuestionNode(IN CHAR8 *Name, IN CHAR8 *VarIdStr,
                                   IN UINT32 BitMask) {
  mName = NULL;
  mVarIdStr = NULL;
...
SVfrQuestionNode::~SVfrQuestionNode(VOID) {
  if (mName != NULL) {
    delete mName;
  }
...
// Reset to init state
//
VOID CVfrQuestionDB::ResetInit(IN VOID) {
  UINT32 Index;
  SVfrQuestionNode *pNode;
...
VOID CVfrQuestionDB::PrintAllQuestion(VOID) {
...
EFI_VFR_RETURN_CODE
CVfrQuestionDB::RegisterQuestion(IN CHAR8 *Name, IN CHAR8 *VarIdStr,
  ...
  gCFormPkg.DoPendingAssign(VarIdStr, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));

  return VFR_RETURN_SUCCESS;
}

VOID CVfrQuestionDB::RegisterOldDateQuestion(
    IN CHAR8 *YearVarId, IN CHAR8 *MonthVarId, IN CHAR8 *DayVarId,
    IN OUT EFI_QUESTION_ID &QuestionId) {
  SVfrQuestionNode *pNode[3] = {
      NULL,
  };
  UINT32 Index;
  ...
  if ((pNode[0] = new SVfrQuestionNode(NULL, YearVarId, DATE_YEAR_BITMASK)) ==
      NULL) {
    goto Err;
  }
  if ((pNode[1] = new SVfrQuestionNode(NULL, MonthVarId, DATE_MONTH_BITMASK)) ==
      NULL) {
    goto Err;
  }
  if ((pNode[2] = new SVfrQuestionNode(NULL, DayVarId, DATE_DAY_BITMASK)) ==
      NULL) {
    goto Err;
  }
  ...
  gCFormPkg.DoPendingAssign(YearVarId, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
  gCFormPkg.DoPendingAssign(MonthVarId, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
  gCFormPkg.DoPendingAssign(DayVarId, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
...
VOID CVfrQuestionDB::RegisterNewDateQuestion(
    IN CHAR8 *Name, IN CHAR8 *BaseVarId, IN OUT EFI_QUESTION_ID &QuestionId) {
  SVfrQuestionNode *pNode[3] = {
      NULL,
  };
  UINT32 Len;
  ...
  if ((pNode[0] = new SVfrQuestionNode(Name, VarIdStr[0], DATE_YEAR_BITMASK)) ==
      NULL) {
    goto Err;
  }
  if ((pNode[1] = new SVfrQuestionNode(Name, VarIdStr[1],
                                       DATE_MONTH_BITMASK)) == NULL) {
    goto Err;
  }
  if ((pNode[2] = new SVfrQuestionNode(Name, VarIdStr[2], DATE_DAY_BITMASK)) ==
      NULL) {
    goto Err;
  }
  ...
  gCFormPkg.DoPendingAssign(VarIdStr[1], (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
  gCFormPkg.DoPendingAssign(VarIdStr[2], (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
...
VOID CVfrQuestionDB::RegisterOldTimeQuestion(
    IN CHAR8 *HourVarId, IN CHAR8 *MinuteVarId, IN CHAR8 *SecondVarId,
    IN OUT EFI_QUESTION_ID &QuestionId) {
  SVfrQuestionNode *pNode[3] = {
      NULL,
  };
  ...
  if ((pNode[0] = new SVfrQuestionNode(NULL, HourVarId, TIME_HOUR_BITMASK)) ==
      NULL) {
    goto Err;
  }
  if ((pNode[1] = new SVfrQuestionNode(NULL, MinuteVarId,
                                       TIME_MINUTE_BITMASK)) == NULL) {
    goto Err;
  }
  if ((pNode[2] = new SVfrQuestionNode(NULL, SecondVarId,
                                       TIME_SECOND_BITMASK)) == NULL) {
    goto Err;
  }
  ...
  gCFormPkg.DoPendingAssign(HourVarId, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
  gCFormPkg.DoPendingAssign(MinuteVarId, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
  gCFormPkg.DoPendingAssign(SecondVarId, (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
...
VOID CVfrQuestionDB::RegisterNewTimeQuestion(
    IN CHAR8 *Name, IN CHAR8 *BaseVarId, IN OUT EFI_QUESTION_ID &QuestionId) {
  SVfrQuestionNode *pNode[3] = {
      NULL,
  };
  ...
  if ((pNode[0] = new SVfrQuestionNode(Name, VarIdStr[0], TIME_HOUR_BITMASK)) ==
      NULL) {
    goto Err;
  }
  if ((pNode[1] = new SVfrQuestionNode(Name, VarIdStr[1],
  ...
  if ((pNode[2] = new SVfrQuestionNode(Name, VarIdStr[2],
                                       TIME_SECOND_BITMASK)) == NULL) {
    goto Err;
  }
...
VOID CVfrQuestionDB::RegisterRefQuestion(IN CHAR8 *Name, IN CHAR8 *BaseVarId,
                                         IN OUT EFI_QUESTION_ID &QuestionId) {
  SVfrQuestionNode *pNode[4] = {
      NULL,
  };
  UINT32 Len;
  CHAR8 *VarIdStr[4] = {
  ...
  gCFormPkg.DoPendingAssign(VarIdStr[2], (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
  gCFormPkg.DoPendingAssign(VarIdStr[3], (VOID *)&QuestionId,
                            sizeof(EFI_QUESTION_ID));
...
EFI_VFR_RETURN_CODE
CVfrQuestionDB::UpdateQuestionId(IN EFI_QUESTION_ID QId,
  ...
  gCFormPkg.DoPendingAssign(pNode->mVarIdStr, (VOID *)&NewQId,
                            sizeof(EFI_QUESTION_ID));

  return VFR_RETURN_SUCCESS;
}

VOID CVfrQuestionDB::GetQuestionId(IN CHAR8 *Name, IN CHAR8 *VarIdStr,
...
EFI_VFR_RETURN_CODE
CVfrQuestionDB::FindQuestion(IN EFI_QUESTION_ID QuestionId) {
  SVfrQuestionNode *pNode;
...
EFI_VFR_RETURN_CODE
CVfrQuestionDB::FindQuestion(IN CHAR8 *Name) {
  SVfrQuestionNode *pNode;
...
CVfrStringDB::CVfrStringDB() { mStringFileName = NULL; }
...
VOID CVfrStringDB::SetStringFileName(IN CHAR8 *StringFileName) {
  UINT32 FileLen = 0;
...
/**
  Returns TRUE or FALSE whether SupportedLanguages contains the best matching
language from a set of supported languages.

  @param[in]  SupportedLanguages  A pointer to a Null-terminated ASCII string
that contains a set of language codes.
  @param[in]  Language            A variable that contains pointers to
Null-terminated ASCII strings that contain one language codes.

  @retval FALSE   The best matching language could not be found in
SupportedLanguages.
  @retval TRUE    The best matching language could be found in
SupportedLanguages.

**/
BOOLEAN
CVfrStringDB::GetBestLanguage(IN CONST CHAR8 *SupportedLanguages,
                              IN CHAR8 *Language) {
  UINTN CompareLength;
  ...
  //
  for (LanguageLength = 0;
       Language[LanguageLength] != 0 && Language[LanguageLength] != ';';
       LanguageLength++)
    ;
  ...
  while (LanguageLength > 0) {
    ...
    for (Supported = SupportedLanguages; *Supported != '\0';
      ...
      //
      for (CompareLength = 0;
           Supported[CompareLength] != 0 && Supported[CompareLength] != ';';
           CompareLength++)
        ;
      //
    ...
    //
    for (LanguageLength--;
         LanguageLength > 0 && Language[LanguageLength] != '-';
         LanguageLength--)
      ;
  }
...
CHAR8 *CVfrStringDB::GetVarStoreNameFormStringId(IN EFI_STRING_ID StringId) {
  FILE *pInFile = NULL;
  UINT32 NameOffset;
  UINT32 Length;
  UINT8 *StringPtr;
  ...
  while (!GetBestLanguage("en", PkgHeader->Language)) {
    ...
    //
    // If can't find string package base on language, just return the first
    // string package.
    //
...
EFI_STATUS
CVfrStringDB::FindStringBlock(IN UINT8 *StringData, IN EFI_STRING_ID StringId,
  ...
  while (*BlockHdr != EFI_HII_SIBT_END) {
    ...
    case EFI_HII_SIBT_STRINGS_SCSU:
      memcpy(&StringCount, BlockHdr + sizeof(EFI_HII_STRING_BLOCK),
             sizeof(UINT16));
      StringTextPtr =
          BlockHdr + sizeof(EFI_HII_SIBT_STRINGS_SCSU_BLOCK) - sizeof(UINT8);
    ...
    case EFI_HII_SIBT_STRINGS_SCSU_FONT:
      memcpy(&StringCount,
             ...
             sizeof(UINT16));
      StringTextPtr = BlockHdr + sizeof(EFI_HII_SIBT_STRINGS_SCSU_FONT_BLOCK) -
                      sizeof(UINT8);
      BlockSize += StringTextPtr - BlockHdr;
    ...
    case EFI_HII_SIBT_STRING_UCS2:
      ...
      StringTextPtr = BlockHdr + Offset;
      //
      // Use StringSize to store the size of the specified string, including the
      // NULL terminator.
      //
    ...
    case EFI_HII_SIBT_STRING_UCS2_FONT:
      ...
      StringTextPtr = BlockHdr + Offset;
      //
      // Use StrSize to store the size of the specified string, including the
      // NULL terminator.
      //
      StringSize = GetUnicodeStringTextSize(StringTextPtr);
      BlockSize += Offset + StringSize;
    ...
    case EFI_HII_SIBT_DUPLICATE:
      if (CurrentStringId == StringId) {
        ...
        //
        memcpy(&StringId, BlockHdr + sizeof(EFI_HII_STRING_BLOCK),
               sizeof(EFI_STRING_ID));
        CurrentStringId = 1;
        BlockSize = 0;
      } else {
    ...
    case EFI_HII_SIBT_SKIP2:
      memcpy(&SkipCount, BlockHdr + sizeof(EFI_HII_STRING_BLOCK),
             sizeof(UINT16));
      CurrentStringId = (UINT16)(CurrentStringId + SkipCount);
    ...
    case EFI_HII_SIBT_EXT1:
      memcpy(&Length8, BlockHdr + sizeof(EFI_HII_STRING_BLOCK) + sizeof(UINT8),
             sizeof(UINT8));
      BlockSize += Length8;
      break;
    ...
    case EFI_HII_SIBT_EXT4:
      memcpy(&Length32, BlockHdr + sizeof(EFI_HII_STRING_BLOCK) + sizeof(UINT8),
             sizeof(UINT32));
    ...
    if (StringId > 0 && StringId != (EFI_STRING_ID)(-1)) {
      ...
      *BlockType = *BlockHdr;
      ...
      if (StringId == CurrentStringId - 1) {
        ...
        //
        if (*BlockType == EFI_HII_SIBT_SKIP2 ||
            *BlockType == EFI_HII_SIBT_SKIP1) {
...
UINT32
CVfrStringDB::GetUnicodeStringTextSize(IN UINT8 *StringSrc) {
  UINT32 StringSize;
  CHAR16 *StringPtr;
