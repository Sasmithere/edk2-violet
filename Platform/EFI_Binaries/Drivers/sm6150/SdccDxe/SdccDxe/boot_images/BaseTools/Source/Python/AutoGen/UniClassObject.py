...
class UniFileClassObject(object):
    ...
    def OpenUniFile(self, FileName):
        ...
        UniFile = StringIO.BytesIO(FileIn)
    ...
    def VerifyUcs2Data(self, FileIn, FileName, Encoding):
        ...
        try:
            ...
            for Char in FileDecoded:
                if ord(Char) > 0xFFFF:
                    raise UnicodeError("Non-UCS-2 character: %r" % Char)
        except:
            UniFile = StringIO.BytesIO(FileIn)
            Info = codecs.lookup(Encoding)
            (Reader, Writer) = (Info.streamreader, Info.streamwriter)
    ...
    def PreProcess(self, File):
        ...
        for Line in FileIn:
            ...
            while (StartPos != -1):
                EndPos = Line.find('\\', StartPos + 1, StartPos + 7)
                if EndPos != -1 and EndPos - StartPos == 6 :
                    if re.match('[a-fA-F0-9]{4}', Line[StartPos + 2 : EndPos], re.UNICODE):
                        EndStr = Line[EndPos: ]
                        UniStr = chr(int(Line[StartPos + 2 : EndPos], 16))
                        if EndStr.startswith('\\x') and len(EndStr) >= 7:
                            if EndStr[6] == '\\' and re.match('[a-fA-F0-9]{4}', EndStr[2 : 6], re.UNICODE):
                                Line = Line[0 : StartPos] + UniStr + EndStr
                        else:
                            Line = Line[0 : StartPos] + UniStr + EndStr[1:]
                StartPos = Line.find('\\x', StartPos)
