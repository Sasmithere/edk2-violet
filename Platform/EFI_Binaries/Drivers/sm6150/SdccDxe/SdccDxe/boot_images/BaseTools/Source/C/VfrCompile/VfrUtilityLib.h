...
public:
  virtual VOID WriteLine(IN FILE *, IN UINT32, IN CONST CHAR8 *, IN CHAR8 *,
                         IN UINT32);
  virtual VOID WriteEnd(IN FILE *, IN UINT32, IN CONST CHAR8 *, IN CHAR8 *,
                        IN UINT32);
};
...
public:
  ...
  SConfigItem(IN CHAR8 *, IN EFI_GUID *, IN CHAR8 *, IN UINT8, IN UINT16,
              IN UINT16, IN EFI_IFR_TYPE_VALUE);
  virtual ~SConfigItem();
};
...
public:
  ...
  virtual UINT8 Write(IN CONST CHAR8, IN CHAR8 *, IN EFI_GUID *, IN CHAR8 *,
                      IN UINT8, IN UINT16, IN UINT32, IN EFI_IFR_TYPE_VALUE);
#if 0
...
public:
  EFI_VFR_RETURN_CODE Pack(IN UINT32, IN UINT8, IN CHAR8 *Identifier = NULL,
...
public:
  ...
  EFI_VFR_RETURN_CODE ExtractFieldNameAndArrary(IN CHAR8 *&, OUT CHAR8 *,
                                                OUT UINT32 &);
...
public:
  SVfrVarStorageNode(IN EFI_GUID *, IN CHAR8 *, IN EFI_VARSTORE_ID,
                     IN EFI_STRING_ID, IN UINT32, IN BOOLEAN Flag = TRUE);
  SVfrVarStorageNode(IN EFI_GUID *, IN CHAR8 *, IN EFI_VARSTORE_ID,
                     IN SVfrDataType *, IN BOOLEAN Flag = TRUE);
...
#define EFI_FREE_VARSTORE_ID_BITMAP_SIZE                                       \
...
private:
  EFI_VARSTORE_ID
  GetFreeVarStoreId(EFI_VFR_VARSTORE_TYPE VarType = EFI_VFR_VARSTORE_BUFFER);
  ...
  EFI_VARSTORE_ID CheckGuidField(IN SVfrVarStorageNode *, IN EFI_GUID *,
                                 IN BOOLEAN *, OUT EFI_VFR_RETURN_CODE *);

public:
  CVfrDataStorage();
  ~CVfrDataStorage();

  SVfrVarStorageNode *GetBufferVarStoreList() { return mBufferVarStoreList; }
  SVfrVarStorageNode *GetEfiVarStoreList() { return mEfiVarStoreList; }
  ...
  EFI_VFR_RETURN_CODE DeclareEfiVarStore(IN CHAR8 *, IN EFI_GUID *,
                                         IN EFI_STRING_ID, IN UINT32,
                                         IN BOOLEAN Flag = TRUE);

  EFI_VFR_RETURN_CODE DeclareBufferVarStore(IN CHAR8 *, IN EFI_GUID *,
                                            IN CVfrVarDataTypeDB *, IN CHAR8 *,
                                            IN EFI_VARSTORE_ID,
                                            IN BOOLEAN Flag = TRUE);
  ...
  EFI_VFR_RETURN_CODE GetVarStoreByDataType(IN CHAR8 *,
                                            OUT SVfrVarStorageNode **,
                                            IN EFI_GUID *VarGuid = NULL);

  EFI_VFR_RETURN_CODE GetBufferVarStoreDataTypeName(IN EFI_VARSTORE_ID,
                                                    OUT CHAR8 **);
...
#define EFI_FREE_QUESTION_ID_BITMAP_SIZE                                       \
...
public:
  ...
  EFI_VFR_RETURN_CODE RegisterQuestion(IN CHAR8 *, IN CHAR8 *,
                                       IN OUT EFI_QUESTION_ID &);
  VOID RegisterOldDateQuestion(IN CHAR8 *, IN CHAR8 *, IN CHAR8 *,
                               IN OUT EFI_QUESTION_ID &);
  VOID RegisterNewDateQuestion(IN CHAR8 *, IN CHAR8 *,
                               IN OUT EFI_QUESTION_ID &);
  VOID RegisterOldTimeQuestion(IN CHAR8 *, IN CHAR8 *, IN CHAR8 *,
                               IN OUT EFI_QUESTION_ID &);
  VOID RegisterNewTimeQuestion(IN CHAR8 *, IN CHAR8 *,
                               IN OUT EFI_QUESTION_ID &);
  ...
  VOID GetQuestionId(IN CHAR8 *, IN CHAR8 *, OUT EFI_QUESTION_ID &,
                     OUT UINT32 &, OUT EFI_QUESION_TYPE *QType = NULL);
  ...
  VOID SetCompatibleMode(IN BOOLEAN Mode) { VfrCompatibleMode = Mode; }
};
...
struct SVfrDefaultStoreNode {
  ...
  SVfrDefaultStoreNode(IN EFI_IFR_DEFAULTSTORE *, IN CHAR8 *, IN EFI_STRING_ID,
                       IN UINT16);
...
public:
  ...
  EFI_VFR_RETURN_CODE RegisterDefaultStore(IN CHAR8 *, IN CHAR8 *,
                                           IN EFI_STRING_ID, IN UINT16);
  EFI_VFR_RETURN_CODE ReRegisterDefaultStoreById(IN UINT16, IN CHAR8 *,
                                                 IN EFI_STRING_ID);
  BOOLEAN DefaultIdRegistered(IN UINT16);
  EFI_VFR_RETURN_CODE GetDefaultId(IN CHAR8 *, OUT UINT16 *);
  EFI_VFR_RETURN_CODE BufferVarStoreAltConfigAdd(IN EFI_VARSTORE_ID,
...
private:
  ...
  EFI_STATUS FindStringBlock(IN UINT8 *StringData, IN EFI_STRING_ID StringId,
                             OUT UINT32 *StringTextOffset,
                             OUT UINT8 *BlockType);

  UINT32 GetUnicodeStringTextSize(IN UINT8 *StringSrc);

  BOOLEAN GetBestLanguage(IN CONST CHAR8 *SupportedLanguages,
                          IN CHAR8 *Language);
...
public:
  ...
  VOID SetStringFileName(IN CHAR8 *StringFileName);

  CHAR8 *GetVarStoreNameFormStringId(IN EFI_STRING_ID StringId);
};
