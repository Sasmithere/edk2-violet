...
\t${END}@cd $(MODULE_BUILD_DIR)\n\n''')
    ...
    def _CreateTemplateDict(self):
        ...
        for k, v in self._AutoGenObject.Module.Defines.items():
            if k not in list(self._AutoGenObject.Macros.keys()):
                self._AutoGenObject.Macros[k] = v

        if 'MODULE_ENTRY_POINT' not in list(self._AutoGenObject.Macros.keys()):
            self._AutoGenObject.Macros['MODULE_ENTRY_POINT'] = ModuleEntryPoint
        if 'ARCH_ENTRY_POINT' not in list(self._AutoGenObject.Macros.keys()):
            self._AutoGenObject.Macros['ARCH_ENTRY_POINT'] = ArchEntryPoint
        if 'IMAGE_ENTRY_POINT' not in list(self._AutoGenObject.Macros.keys()):
        ...
        if RespDict:
            RespFileListContent = ''
            for Resp in list(RespDict.keys()):
                ...
                for Target in BuildTargets:
                    ...
                    for SingleCommand in CommandList:
                        ...
                        if len(SingleCommandList) > 0:
                            for Flag in list(FlagDict.keys()):
                        ...
                        if Tool:
                            ...
                            for item in SingleCommandList[1:]:
                                if FlagDict[Tool]['Macro'] in item:
                                    ...
                                    for Option in list(self._AutoGenObject.BuildOption.keys()):
                                    ...
                                    while(Str.find('$(') != -1):
                                        for macro in list(self._AutoGenObject.Macros.keys()):
                                            MacroName = '$('+ macro + ')'
    ...
    def GetDependencyList(self, File, ForceList, SearchPathList):
        ...
        while len(FileStack) > 0:
            ...
            if F in DepDb:
                ...
                if FileContent.startswith(b'\xff\xfe') or FileContent.startswith(b'\xfe\xff'):
