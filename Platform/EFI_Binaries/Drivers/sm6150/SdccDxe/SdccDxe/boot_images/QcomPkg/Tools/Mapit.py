          ...
          for seg in sorted(segments):
              if seg in lib_sizes[sec][lib]:
                  ...
                  if (seg == "CODE" or seg == "RO" or seg == "RW"):
                      rom += segments[seg]
                  print(lib_sizes[sec][lib][seg], end=' ') 
                  print(column_padding(str(lib_sizes[sec][lib][seg]), col_width), end=' ')
              else: 
  ...
  with open(filename + ".txt") as fh:
    for line in fh:
      ...
      if m :
        ...
        mem_addr_start = int(m.group(2), 0)
        mem_addr_end = int(m.group(3), 0)
        mem_reg_size = mem_addr_end - mem_addr_start
        mem_reg_used[mem_reg_name] = 0
        
        # Print Memory Region Information
        print("Memory Region : " + mem_reg_name, column_padding("Memory Region : " + mem_reg_name, col_width), end=' ')
        print("Start Addr : " + hex(mem_addr_start), column_padding("Start Addr : " + hex(mem_addr_start), col_width), end=' ')
...
def print_sec_summary(lib_sizes, mem_addr_start, mem_addr_end, mem_reg_used, mem_reg_name):
  ...
  # print headers
  print("Section", column_padding("Section", 30), end=' ')
  print("Address", column_padding("Address", col_width), end=' ')
  print("Size", column_padding("Size", col_width), end=' ')
  for key in sorted(segments):
    print(key, column_padding(key, col_width), end=' ')
  ...
  for sec in lib_sizes:
    sec_addr = int(hex(sec.address), 0)
    sec_size = int(hex(sec.size), 0)
    
    if (not((sec_addr > mem_addr_start) and ((sec_addr + sec_size) < (mem_addr_end)))):
      continue;
    
    print(sec.name, column_padding(sec.name, 30), end=' ')
    print(hex(sec.address), column_padding(hex(sec.address), col_width), end=' ')
    print(sec.size, column_padding(str(sec.size), col_width), end=' ')
