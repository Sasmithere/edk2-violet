...
DEFAULT_TARGET_REGEX = r"^([a-zA-Z]{3,4})(\d([\d|x|X]{2,3}))Pkg$"
      ...
      for release_mode in options.release.split(','):
        ...
        for image in images:
          ...
          if("SDM670Pkg" == variant_flavor.target):
            print("[buildit.py] Generating XBLLoader Memory Usage as XBLL_Memoy_Usage.txt... ", end=' ')
...
def create_image(options,variant_flavor,build_dirt,tools_dirt):
  ...
  for i in range(len(build_dirt['CreateImages'])):
    if build_dirt['CreateImages'][i]['Name'] == str(variant_flavor.image):
...
def process_componet(options,variant_flavor,component,process_dirt,build_dirt,tools_dirt):
  ...
  for i in range(len(build_dirt['Components'])):
    if build_dirt['Components'][i]['Name'] == str(component):
  ...
  for i in range(len(component_dirt['Process'])):
    ...
    if build_cmd[0] == "build":
      ...
      variant_flavor.aarch = component_dirt['Process'][i]['AArch']
      ARGUMENTS['$AARCH']= variant_flavor.aarch
      if ((options.components and component_dirt['Name'].upper() in options.components.upper()) or options.components == "") and not check_componets_status(dsc_path, variant_flavor.release_mode):
      ...
      if tool_dirt != "":
        ...
        tool_cmd.append(cprocess_dirt['ToolChain'])
...
def get_cmd_params(options,variant_flavor,cprocess_dirt,tool_dirt):
  ...
  if 'Params' in tool_dirt and tool_dirt:
    for i in range(len(tool_dirt['Params'])):
      str1 = replace_arguments(tool_dirt['Params'][i]) 
      build_cmd.append(str1)
      if tool_dirt['Params'][i] == "-t":
        variant_flavor.compiler = tool_dirt['Params'][i+1]
...
def setup_environment():
  ...
  if platform.system() == "Windows":
    ...
    edk_output = subprocess.check_output('edksetup.bat --nt32 && echo ' + EDK_ENV_FLAG + ' && set', shell=True, text=True)
    ...
    proc = subprocess.Popen(  edk_command, stdout=subprocess.PIPE, text=True)
...
def create_build_version(build_version):
  ...
  if os.path.isfile(build_version_file) and build_version == "0":
    print(r"[buildex.py] QcomPkg\Include\BuildVersion.h already exists.")
    print("[buildex.py] No build version specified. Will use existing version.")
    return
  
  print(r"[buildex.py] Creating QcomPkg\Include\BuildVersion.h with version: " + build_version)
  try:
...
def find_tag_in_descriptor_file(dsc, section, tag):
  with open(os.path.join(os.environ["WORKSPACE"], dsc), 'r') as f:
    ...
    for line in f.readlines():
      ...
      if section:
        section_search = re.search(r"\[(\S+)\]", line)
        if section_search:
...
def generate_merge_item(merge_dict,dsc_path,variant_flavor,process_dirt):
  ...
  with open(os.path.join(os.environ["WORKSPACE"],dsc_path), 'r') as d:
    ...
    line = d.readline()
    while line and not re.search(r'\[Components.common\]', line):
      line = d.readline()
    
    # If we reached EOF without finding the section, return None
    if not line:
      return None
    ...
    for line in d.readlines():
      ...
      if bracket_count == 0:
        ...
        if line.find(".inf") != -1 and line.find("{") != -1:
          bracket_count = 1
          inf = re.search(r"(\S+.inf)", line).group(1)
