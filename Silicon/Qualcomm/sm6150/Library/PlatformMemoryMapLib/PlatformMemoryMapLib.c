...
static ARM_MEMORY_REGION_DESCRIPTOR_EX gDeviceMemoryDescriptorEx[] = {
    ...
    /* DDR Regions directly derived from adb_dump / Linux iomem */
#ifdef USE_DISPLAYDXE
    {"Kernel",            0x80000000, 0x03300000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, WRITE_BACK_XN},
    {"Display Reserved",  0x83300000, 0x02400000, AddMem, MEM_RES, SYS_MEM_CAP, Reserv, WRITE_THROUGH_XN},
#else
    {"Kernel",            0x80000000, 0x05700000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, WRITE_BACK_XN},
#endif
    {"hyp_region",        0x85700000, 0x00600000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, NS_DEVICE},
    {"HLOS 1",            0x85D00000, 0x00100000, AddMem, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"xbl_aop_mem",       0x85E00000, 0x00120000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, NS_DEVICE},
    {"AOP CMD DB",        0x85F20000, 0x00020000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"HLOS 2",            0x85F40000, 0x000BF000, AddMem, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"sec_apps_region",   0x85FFF000, 0x00001000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, NS_DEVICE},
    {"SMEM",              0x86000000, 0x00200000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"removed_region",    0x86200000, 0x02D00000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, NS_DEVICE},
    {"HLOS 3",            0x88F00000, 0x01C00000, AddMem, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"camera_region",     0x8AB00000, 0x00500000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"modem_region",      0x8B000000, 0x07E00000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"pil_video_region",  0x92E00000, 0x00500000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"wlan_msa_region",   0x93300000, 0x00200000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"cdsp_regions",      0x93500000, 0x01E00000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"pil_adsp_region",   0x95300000, 0x01E00000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"ips_fw_region",     0x97100000, 0x00010000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"ipa_gsi_region",    0x97110000, 0x00005000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"gpu_region",        0x97115000, 0x00002000, AddMem, MEM_RES, UNCACHEABLE, Reserv, UNCACHED_UNBUFFERED_XN},
    {"Ram Partition",     0x97117000, 0x04AE9000, AddMem, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN}, 
    {"Sched Heap",        0x9BC00000, 0x00400000, AddMem, SYS_MEM, SYS_MEM_CAP, BsData, WRITE_BACK_XN},
#ifndef USE_DISPLAYDXE
    /* Splash screen framebuffer fits from 9C000000 to 9E400000 perfectly. */
    {"Display Reserved",  0x9C000000, 0x02400000, AddMem, MEM_RES, SYS_MEM_CAP, Reserv, WRITE_THROUGH_XN},
#endif

    {"QSEECOM",           0x9E400000, 0x01400000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, NS_DEVICE},
    /* EDK2 structures at High Memory matching original DSC limits (0x9F800000 and 0xC0000000) */
    ...
    {"HLOS 4",            0xA0000000, 0x00400000, AddMem, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"LAST KMSG",         0xA0400000, 0x00400000, AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, WRITE_BACK_XN},
    {"Ram Partition",     0xA0800000, 0x1F800000, Mem4G, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"DXE Heap",          0xC0000000, 0x0E000000, Mem4G, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"Ram Partition",     0xCE000000, 0x02000000, Mem4G, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    {"Ram Partition",     0xD0000000, 0x30000000, Mem4G, SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
/*===============================================================================================================*/
    // 4GB Memory
    {"RAM Partition",    0x100000000, 0x7DD00000,  Mem4G,  SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
/*===============================================================================================================*/
    // 6GB Memory
    // {"RAM Partition",    0x100000000, 0x7DD00000,  Mem6G,  SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
    // {"RAM Partition",    0x17DD00000, 0x80000000,  Mem6G,  SYS_MEM, SYS_MEM_CAP, Conv,   WRITE_BACK_XN},
/*===============================================================================================================*/
    ...
    {"SDC1_CMDQ",         0x007C5000, 0x00001000,  AddDev, MMAP_IO, UNCACHEABLE, MmIO,   NS_DEVICE},
    {"SDC1_ICE",         0x007C8000, 0x00008000,  AddDev, MMAP_IO, UNCACHEABLE, MmIO,   NS_DEVICE},
    ...
    {"APSS_WDT_TMR1",     0x17C10000, 0x00010000,  AddDev, MMAP_IO, UNCACHEABLE, MmIO,   NS_DEVICE},
