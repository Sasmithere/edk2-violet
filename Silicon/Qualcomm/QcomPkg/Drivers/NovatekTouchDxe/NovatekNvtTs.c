...
EFI_STATUS EFIAPI SimplePReset(
    IN EFI_SIMPLE_POINTER_PROTOCOL *This, IN BOOLEAN ExtendedVerification);

EFI_STATUS EFIAPI SimplePGetState(
    IN EFI_SIMPLE_POINTER_PROTOCOL  *This,
    IN OUT EFI_SIMPLE_POINTER_STATE *State);

VOID EFIAPI SimplePWaitForInput(IN EFI_EVENT Event, IN VOID *Context);
...
VOID NvtStopCrcReboot(NVT_INTERNAL_DATA *Instance)
{
  ...
  if ((Buf[1] == 0xFC) ||
    ...
    for (Retry = 5; Retry > 0; Retry--) {
      ...
      NvtSetSlaveAddress(Instance, I2C_HW_Address);
      NvtI2cWriteRaw(Instance, Buf, 2);
      MicroSecondDelay(1000); // Increased delay
      ...
      if (!EFI_ERROR(Status) && Buf[1] == 0xA5) {
        // DEBUG(
        //     (EFI_D_ERROR, "NVT: CRC Reboot Stopped (0xA5) after %d
        //     retries\n",
        //      6 - Retry));
        break;
      }
    ...
    if (Retry == 0) {
      // DEBUG(
      //     (EFI_D_ERROR, "NVT: CRC reboot could not be stopped!
      //     buf[1]=0x%02X\n",
      //      Buf[1]));
    }
  }
...
EFI_STATUS NvtCheckChipVerTrim(NVT_INTERNAL_DATA *Instance)
{
  ...
  for (Retry = 5; Retry > 0; Retry--) {
    ...
    for (List = 0;
      ...
      if (i == NVT_ID_BYTE_MAX) {
        ...
        Instance->MemMap = trim_id_table[List].mmap; // FIX: Initialize MemMap
                                                     // pointer! DEBUG((
  ...
  // DEBUG((EFI_D_ERROR, "NVT: Chip Identify Failed - Using Default
  // 0x21C00\n"));
...
NvtI2cReceive(IN NVT_INTERNAL_DATA *Instance, OUT UINT8 *Data, IN UINT32 Length)
{
  ...
  for (UINT32 i = 0; i < Length; i++) {
    ...
    if (i % 10 == 0) {
      MicroSecondDelay(100); // Prevent lockup
    }
  }
...
VOID NvtI2cScan(NVT_INTERNAL_DATA *Instance)
{
  ...
  for (I = 0x00; I < 0x7F; I++) {
    ...
    if (EFI_ERROR(Status)) {
      // DEBUG(
      //     (EFI_D_ERROR, "\nNovatekNvtTsDxe: FOUND Device at Address:
      //     0x%02X\n",
      //      I));
    }
  }
...
NVT_INTERNAL_DATA mNvtInstanceTemplate =
                      {
                          ...
                          {0}, // AbsPointerMode
                          {SimplePReset, SimplePGetState, NULL,
                           NULL}, // SimplePointerProtocol
                          {0},    // SimplePointerMode
                          NULL,   // PollingTimer
                          NULL,   // TouchWorker
                          FALSE,  // Initialized
...
EFI_STATUS NvtDetectChip(NVT_INTERNAL_DATA *Instance)
{
  ...
  // DEBUG((EFI_D_ERROR, "NVT: DetectChip - Delegating to
  // CheckChipVerTrim...\n"));
  ...
  if (EFI_ERROR(Status)) {
    // DEBUG((EFI_D_ERROR, "NVT: CheckChipVerTrim Failed or Chip Not
    // Found.\n"));
...
NvtChangeMode(IN NVT_INTERNAL_DATA *Instance, IN UINT8 Mode)
{
  ...
  if (Mode == NORMAL_MODE) {
    ...
    MicroSecondDelay(20000); // 20ms delay
  }
...
VOID NvtDrainFifo(NVT_INTERNAL_DATA *Instance)
{
  ...
  for (Retry = 0; Retry < 10; Retry++) {
    ...
    MicroSecondDelay(10000); // 10ms between drain reads
  }
...
//   //DEBUG((EFI_D_ERROR, "NVT: Using Android defaults (firmware is
//   broken)\n"));
...
//   if (EFI_ERROR(Status)) {
//     //DEBUG((EFI_D_ERROR, "NVT: Mode change failed - touch may not
//     work!\n"));
    ...
    IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath)
{
  ...
  // This breaks the CRC reboot loop seen in dmesg
  // DEBUG((EFI_D_ERROR, "NVT: Forcing reset to idle (0xA5) with
  // Handshake...\n"));
  ...
  if (!HandshakeSuccess) {
    // DEBUG((EFI_D_ERROR, "NVT: Handshake failed, proceeding but
    // unstable...\n"));
  }
  ...
  MicroSecondDelay(100000); // Wait 100ms for recalibration
  ...
  if (EFI_ERROR(Status)) {
    // DEBUG((EFI_D_ERROR, "NVT: DetectChip failed, using defaults
    // (0x21C00)\n"));
  ...
  // Setup Simple Pointer Protocol
  Instance->SimplePointerProtocol.Reset    = SimplePReset;
  Instance->SimplePointerProtocol.GetState = SimplePGetState;
  Instance->SimplePointerProtocol.Mode     = &Instance->SimplePointerMode;
  Instance->SimplePointerMode.ResolutionX  = 1; // 1 count/mm? Dummy.
  Instance->SimplePointerMode.ResolutionY  = 1;
  Instance->SimplePointerMode.ResolutionZ  = 1;
  Instance->SimplePointerMode.LeftButton   = FALSE;
  Instance->SimplePointerMode.RightButton  = FALSE;

  Status = gBS->CreateEvent(
      EVT_NOTIFY_WAIT, TPL_NOTIFY, SimplePWaitForInput, Instance,
  ...
  Status = gBS->InstallMultipleProtocolInterfaces(
      ...
      &Instance->PointProtocol, &gEfiSimplePointerProtocolGuid,
      &Instance->SimplePointerProtocol, NULL);
...
// --- SIMPLE POINTER PROTOCOL IMPLEMENTATION ---
...
/**
  Reset the Simple Pointer Device
...
{
  NVT_INTERNAL_DATA *Instance =
      CR(This, NVT_INTERNAL_DATA, SimplePointerProtocol,
         NVT_TCH_INSTANCE_SIGNATURE);

  Instance->AccumulatedScrollZ = 0;
  Instance->IsScrolling        = FALSE;
  Instance->ScrollFinger1ID    = -1;
  Instance->ScrollFinger2ID    = -1;

  return EFI_SUCCESS;
}

/**
  Get Simple Pointer State
    ...
    IN OUT EFI_SIMPLE_POINTER_STATE *State)
{
  ...
  if (Instance->AccumulatedScrollZ == 0) {
    return EFI_NOT_READY;
  }

  State->RelativeMovementX = 0;
  State->RelativeMovementY = 0;
  State->RelativeMovementZ = Instance->AccumulatedScrollZ;
...
/**
  Process Two-Finger Scroll Gestures

  Scans the buffer for two valid touch points moving in unison.
  Updates AccumulatedScrollZ.

  @param Instance    Driver Instance
**/
VOID NvtProcessGesture(NVT_INTERNAL_DATA *Instance)
{
  UINT32 ActiveFingers = 0;
  INT32  Fingers[2]    = {-1, -1};
  UINT16 CurX[2]       = {0, 0};
  UINT16 CurY[2]       = {0, 0};
  UINT32 i;
  ...
  for (i = 0; i < 10; i++) {
    UINT32 Position = 1 + (6 * i);

    if (Position + 5 > POINT_DATA_LEN)
      break;

    // Dump First 2 Slots for Debug
    // if (i < 2 && Instance->IsTouched) {
    //   DEBUG(
    //       (EFI_D_ERROR, "NVT: Slot %d: %02X %02X %02X %02X %02X %02X\n", i,
    //        Instance->Buf[Position], Instance->Buf[Position + 1],
    //        Instance->Buf[Position + 2], Instance->Buf[Position + 3],
    //        Instance->Buf[Position + 4], Instance->Buf[Position + 5]));
    // }

    UINT8 PacketId = (Instance->Buf[Position] >> 3) & 0x1F;
    if (PacketId == 0 || PacketId > 10)
      continue;

    UINT8 Status = Instance->Buf[Position] & 0x07;
    if (Status == 0x01 || Status == 0x02) {
      if (ActiveFingers < 2) {
        Fingers[ActiveFingers] = i;
        UINT16 RawX            = (UINT16)((Instance->Buf[Position + 1] << 4) |
                               (Instance->Buf[Position + 3] >> 4));
        UINT16 RawY            = (UINT16)((Instance->Buf[Position + 2] << 4) |
                               (Instance->Buf[Position + 3] & 0x0F));
        // Linux Logic: Reject Out of Bounds (Ghost Touch Filtering)
        // Check BEFORE transformation
        if (RawX > Instance->AbsPointerMode.AbsoluteMaxX ||
            RawY > Instance->AbsPointerMode.AbsoluteMaxY) {
          // //DEBUG((EFI_D_ERROR, "NVT: Slot %d REJECTED: Raw(%d,%d) >
          // Max(%d,%d)\n", i, RawX, RawY,
          // Instance->AbsPointerMode.AbsoluteMaxX,
          // Instance->AbsPointerMode.AbsoluteMaxY));
          continue;
        ...
        CurX[ActiveFingers] = RawX;
        CurY[ActiveFingers] = RawY;
      }
      ActiveFingers++;
    }
  }
  ...
  if (ActiveFingers == 2) {
    if (!Instance->IsScrolling) {
      // DEBUG(
      //     (EFI_D_ERROR, "NVT: Scroll Start: CurY1=%d CurY2=%d\n", CurY[0],
      //      CurY[1]));
      Instance->IsScrolling  = TRUE;
      Instance->LastScrollX1 = CurX[0];
      Instance->LastScrollY1 = CurY[0];
      Instance->LastScrollX2 = CurX[1];
      Instance->LastScrollY2 = CurY[1];
    }
    else {
      INT32 Dy1 = (INT32)CurY[0] - (INT32)Instance->LastScrollY1;
      INT32 Dy2 = (INT32)CurY[1] - (INT32)Instance->LastScrollY2;

      // //DEBUG((EFI_D_ERROR, "NVT: Scroll Update: Dy1=%d Dy2=%d\n", Dy1,
      // Dy2));

#define SCROLL_THRESHOLD 5
      if ((Dy1 * Dy2 > 0) && (ABS1(Dy1) > SCROLL_THRESHOLD) &&
          (ABS1(Dy2) > SCROLL_THRESHOLD)) {
        INT32 AvgDelta = (Dy1 + Dy2) / 2;
#define SCROLL_SCALE 40
        // Invert Direction: Drag Up (-Dy) -> Scroll Up (+Z)
        INT32 Steps = -(AvgDelta) / SCROLL_SCALE;

        if (Steps != 0) {
          // DEBUG(
          //     (EFI_D_ERROR, "NVT: Scroll STEP: %d (AvgDelta=%d)\n", Steps,
          //      AvgDelta));
          Instance->AccumulatedScrollZ += Steps;
          Instance->LastScrollX1 = CurX[0];
          Instance->LastScrollY1 = CurY[0];
          Instance->LastScrollX2 = CurX[1];
          Instance->LastScrollY2 = CurY[1];
        ...
        // //DEBUG((EFI_D_ERROR, "NVT: Scroll Threshold Fail/Opposite: Dy1=%d
        // Dy2=%d\n", Dy1, Dy2));
      }
    }
  }
    ...
    if (Instance->IsScrolling) {
      // //DEBUG((EFI_D_ERROR, "NVT: Scroll End (Fingers=%d)\n",
      // ActiveFingers));
      Instance->IsScrolling = FALSE;
    }
  }
...
VOID EFIAPI SyncPollCallback(IN EFI_EVENT Event, IN VOID *Context)
{
  ...
  // Always process gestures if I2C read was successful (even if parsing
  // failed/NOT_READY)
  if (!EFI_ERROR(Status) || Status == EFI_NOT_READY) {
    NvtProcessGesture(Instance);
    // Signal SimplePointer if scrolling
    if (Instance->AccumulatedScrollZ != 0) {
      gBS->SignalEvent(Instance->SimplePointerProtocol.WaitForInput);
    }
  }
...
/**
  Wait For Input Event (Simple Pointer)
**/
VOID EFIAPI SimplePWaitForInput(IN EFI_EVENT Event, IN VOID *Context)
{
  NVT_INTERNAL_DATA *Instance = (NVT_INTERNAL_DATA *)Context;
...
/**
  Wait For Input Event (Absolute Pointer)
**/
...
VOID NvtConfigI2cPins(NVT_INTERNAL_DATA *Instance)
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    // DEBUG((EFI_D_ERROR, "NVT: Configured GPIO 4 as I2C SDA
    // (GPIO/PullUp)\n"));
  }
...
EFI_STATUS NvtBootloaderReset(NVT_INTERNAL_DATA *Instance)
{
  ...
  MicroSecondDelay(
      35000); // 35ms delay (Matched with Linux for Aftersales support)
...
EFI_STATUS NvtHardReset(NVT_INTERNAL_DATA *Instance)
{
  ...
  // Note: We drive it Low manually, so configuration matters less, but safe
  // to set.
  UINT32 Config =
  ...
  MicroSecondDelay(20000); // Hold for 20ms (Datasheet usually says >10ms)
  ...
  MicroSecondDelay(100000); // 100ms wait for firmware boot
...
EFI_STATUS NvtGetFwInfo(NVT_INTERNAL_DATA *Instance)
{
  ...
  if (Instance->EventBufAddr == 0) {
    // DEBUG((EFI_D_ERROR, "NVT: EventBufAddr is 0! Forcing default
    // 0x21C00\n"));
    Instance->EventBufAddr = 0x21C00;
  }
  ...
  for (RetryCount = 0; RetryCount < 3; RetryCount++) {
    ...
    if (!EFI_ERROR(Status)) {
      ...
      if ((ChecksumValid || ResolutionPlausible) &&
        ...
        DEBUG(
            (EFI_D_ERROR,
             "NVT: Using Firmware Reported Resolution: %dx%d (Checksum: "
             "%02X)\n",
             Instance->IdInfo.AbsXMax, Instance->IdInfo.AbsYMax,
             (Buf[0] + Buf[1])));
        return EFI_SUCCESS;
...
NvtCheckFwResetState(NVT_INTERNAL_DATA *Instance, UINT8 ExpectedState)
{
  ...
  for (Retry = 0; Retry < 100; Retry++) {
    ...
    // RESET_STATE_MAX) Common states: 0xA0 (Init), 0xA2 (Rek Finish), 0xA3
    // (Rek Fail) NOTE: Linux uses dummy byte, so Buf[0] is dummy, Buf[1] is
    // state.
    ...
    MicroSecondDelay(10000); // 10ms between checks
  }
...
NvtPowerUpController(NVT_INTERNAL_DATA *Instance)
{
      ...
      if (!EFI_ERROR(
        ...
        if (AsciiStrCmp(Clocks[i], "gcc_qupv3_wrap0_s1_clk") == 0) {
          ClockProtocol->SetClockFreqHz(
              ...
              NULL);
          // DEBUG((EFI_D_ERROR, "NVT: Set gcc_qupv3_wrap0_s1_clk
          // to 19.2MHz\n"));
  ...
  */
  ...
  DEBUG(
      (EFI_D_ERROR, "NVT: S1 Clock Voted & Enabled. CBCR (0x17274)=0x%08x\n",
       S1Cbcr));
  // 5. Enable Clocks
  ...
  Instance->NvtDevice = Instance->NvtDevice; // Already done in BindingStart?
                                             // This function is PowerUp.
  ...
  if (CbcrVal & BIT(1)) {
    // DEBUG((EFI_D_ERROR, "NVT: Disabling HW Gating (Clearing Bit
    // 1)...\n"));
  ...
  if (CbcrVal & BIT(31)) {
    DEBUG((
        EFI_D_ERROR, "NVT: WARNING: S_AHB Clock is still OFF (Bit 31 set)!\n"));
  }
  ...
  */
  ...
  Status = NvtCheckFwResetState(Instance, RESET_STATE_INIT);
  if (EFI_ERROR(Status)) {
    // DEBUG((EFI_D_ERROR, "NVT: FW Reset State Check Failed! (Device
    // Stuck?)\n"));
  }
  else {
  ...
  if (EFI_ERROR(Status)) {
    ...
    //  return Status; // REMOVED: Soft-fail, Linux nvt_bootloader_reset is
    //  void
  }
  ...
  if (EFI_ERROR(Status)) {
    ...
    NvtBootloaderReset(Instance);
    MicroSecondDelay(35000); // 35ms

    Status = NvtCheckFwResetState(Instance, RESET_STATE_INIT);
    if (EFI_ERROR(Status)) {
      // DEBUG((EFI_D_ERROR, "Nvt: IC init timeout (Ignored, matching
      // Linux)\n"));
  ...
  if (EFI_ERROR(Status)) {
    // DEBUG(
    //     (EFI_D_ERROR, "Nvt: Calibration timeout (Ignored, matching
    //     Linux)\n"));
  ...
  if (EFI_ERROR(Status)) {
    // DEBUG((EFI_D_ERROR, "Nvt: Tuning config partially failed
    // (non-fatal)\n"));
    return EFI_DEVICE_ERROR;
  }
...
NvtGetTouchData(NVT_INTERNAL_DATA *Instance, IN PTOUCH_DATA DataBuffer)
{
  ...
  // irrelevant? Actually, NvtI2cRead (Wrapper) handles the I2C transaction.
  // We want the data from Offset 0x00 (Low Byte of EventBufAddr)
  ...
  // Relaxed Filter: Allow reasonable pressure range
  // Linux uses 0-1024 or similar. 770-785 is too narrow.
...
NvtApplyTuningConfig(IN NVT_INTERNAL_DATA *Instance)
{
  ...
  if (Instance->TuningConfig.Sensitivity > 0) {
    ...
    MicroSecondDelay(5000); // 5ms delay between settings
  }
